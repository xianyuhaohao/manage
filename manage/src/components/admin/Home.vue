  ...
  <div id="Home" class="clearfix">
    ...
    <div class="left fl">
      ...
      <ul class="count clearfix">
        <li class="fl">
          <p>
            <span class="stat">{{trendlist.yesinc}}</span>
            <span :class="[trendlist.yesstate==2?'decline':'trend']">
              {{trendlist.yesratio}}%
              <i :class="[trendlist.yesstate==2?'down':'up']"></i>
            </span>
          </p>
          <p class="describe">
            <i></i>昨日新增量
        ...
        <li class="fl">
          <p>
            <span class="stat">{{trendlist.yesdonenum}}</span>
            <span :class="[trendlist.yesdonenumstate==2?'decline':'trend']">
              {{trendlist.yesdonenumratio}}%
              <i :class="[trendlist.yesdonenumstate==2?'down':'up']"></i>
            </span>
          </p>
          <p class="describe">
            <i></i>昨日成交量
        ...
        <li class="fl">
          <p>
            <span class="stat">{{trendlist.yesdoneprice}}</span>
            <span :class="[trendlist.yesdonepricestate==2?'decline':'trend']">
              {{trendlist.yesdonepriceratio}}%
              <i
                :class="[trendlist.yesdonepricestate==2?'down':'up']"
              ></i>
            </span>
          </p>
          <p class="describe">
        ...
        <li class="fl">
          <p>
            <span class="stat">{{trendlist.weekinc}}</span>
            <span :class="[trendlist.weekstate==2?'decline':'trend']">
              {{trendlist.weekratio}}%
              <i :class="[trendlist.weekstate==2?'down':'up']"></i>
            </span>
          </p>
          <p class="describe">
            <i></i>本周成交金额
        ...
        <li class="fl">
          <p>
            <span class="stat">{{trendlist.sumdone}}</span>
          </p>
          <p class="describe">
            <i></i>总成交量
          </p>
    ...
    <div class="right fl">
      ...
      <div class="count-info">
        ...
        <div class="btn clearfix">
          <p class="month" @click="week" :class="{'active':counton==1}">本周</p>
          <p class="year" @click="month" :class="{'active':counton==2}">本月</p>
          <p class="week" @click="year" :class="{'active':counton==3}">一年</p>
        </div>
        <div class="rank">
...
export default {
  ...
  methods: {
    ...
    week() {
      this.counton = 1;
      this.$post("/api/admin/order/resultCensus", {
        status: "1"
      }).then(res => {
  ...
  mounted() {
    this.drawLine();
    //订单统计
    this.$post("/api/admin/order/orderCensus").then(res => {
      this.trendlist = res.data;
    });
    // 新客户列表
    this.$post("/api/admin/Courseapply/applyList", {
      status: "1",
      page_number:this.currentPage,
      page_size:this.pagesize
    }).then(res => {
      console.log(res, 33333);
      this.kehulist=res.data.list
      this.kehulist.forEach(item=>{
        // Vue.set(item,'teacherID','')
      })
    });
    // 老师列表
    
    this.$post("/api/admin/user/getUserList", {
      status: "0",
      page_number:this.currentPage,
      page_size:this.pagesize
    }).then(res => {
      console.log(res, 33333);
      this.yuangonglist=res.data.list
    });
    // 老师业绩统计
    this.$post("/api/admin/order/resultCensus", {
      status: "1"
